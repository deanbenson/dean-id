...
// --- Source-prefixed warehouse tables -------------------------------------------------------------
// The read-only mirror of Street CRM lives in tables prefixed "street_" so each feed's provenance is
// obvious at a glance (and a second source later can use its own prefix, e.g. payprop_). Two tables are
// deliberately NOT prefixed: enquiries and listings are the live operational tables the public website
// and lead capture read and write directly. phys() maps a logical dataset name to its physical table.
function phys(name) { return (!name || name === "enquiries" || name === "listings") ? name : "street_" + name; }
// One-time, non-destructive migration: rename each existing warehouse table to its street_ name, keeping
// every row (ALTER ... RENAME TO — no DROP of populated tables, ever). The list is derived from the sync
// registry so it can never drift out of step. Runs once, gated by a KV flag; safe to call on any request.
async function migrateStreetTables(env) {
  if (!env || !env.gr_estates || !env.LISTINGS) return;
  let done; try { done = await env.LISTINGS.get("migrate:street_v1"); } catch (_) { return; }
  if (done) return;
  const db = env.gr_estates;
  const have = {};
  try { const r = await db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all(); ((r && r.results) || []).forEach(function (x) { have[x.name] = 1; }); } catch (_) { return; }
  const names = {};
  try { extrasReg().forEach(function (e) { const t = ddlTable(e.ddl); if (t) names[t.replace(/^street_/, "")] = 1; }); } catch (_) {}
  for (const oldN of Object.keys(names)) {
    const newN = "street_" + oldN;
    try {
      if (have[oldN] && !have[newN]) { await db.prepare('ALTER TABLE "' + oldN + '" RENAME TO "' + newN + '"').run(); have[newN] = 1; delete have[oldN]; }
      else if (have[oldN] && have[newN]) {
        // Both exist (e.g. a sync created an empty street_ table before this ran). Never lose data: only
        // replace the street_ table with the old one if street_ is empty and the old one has rows.
        let nc = 0; try { const a = await db.prepare('SELECT COUNT(*) AS n FROM "' + newN + '"').first(); nc = (a && a.n) || 0; } catch (_) {}
        if (nc === 0) { let oc = 0; try { const b = await db.prepare('SELECT COUNT(*) AS n FROM "' + oldN + '"').first(); oc = (b && b.n) || 0; } catch (_) {} if (oc > 0) { try { await db.prepare('DROP TABLE "' + newN + '"').run(); await db.prepare('ALTER TABLE "' + oldN + '" RENAME TO "' + newN + '"').run(); } catch (_) {} } }
      }
    } catch (_) {}
  }
  try { await env.LISTINGS.put("migrate:street_v1", "1"); } catch (_) {}
}
// Run the rename at most once per isolate; after the global migration it's a single cheap KV read.
let _streetMigrated = null;
function ensureStreetTables(env) { if (!_streetMigrated) _streetMigrated = migrateStreetTables(env).catch(function () {}); return _streetMigrated; }
...
async function syncStreetResource(env, name, path, ddl, mapFn, opts) {
  ...
  await ensureStreetTables(env); // rename warehouse tables to their street_ name once, before any create/write
  await migrateSchema(env);
  await backfillContactDetails(env);
...
// One-time backfill: the early contacts mapper read the wrong sub-key for email/telephone (Street nests the
// actual value under .value), so those columns stored null even though the full raw record holds them. Re-map
// straight from the stored raw — no Street re-pull. Guarded by a KV flag so it runs once.
async function backfillContactDetails(env) {
  if (!env || !env.gr_estates) return;
  try { if (env.LISTINGS && (await env.LISTINGS.get("migrate:contacts_value_v1"))) return; } catch (_) { return; }
  const db = env.gr_estates;
  try { await db.prepare("UPDATE street_contacts SET email = json_extract(raw,'$.attributes.email_addresses[0].value'), phone = json_extract(raw,'$.attributes.telephone_numbers[0].value') WHERE raw IS NOT NULL AND (email IS NULL OR email = '')").run(); } catch (_) {}
  try { if (env.LISTINGS) await env.LISTINGS.put("migrate:contacts_value_v1", "1"); } catch (_) {}
}
const VIEW_DDL = "CREATE TABLE IF NOT EXISTS street_viewings (id TEXT PRIMARY KEY, created_at TEXT, start TEXT, end_at TEXT, viewing_type TEXT, status TEXT, address TEXT, branch TEXT, property_id TEXT, raw TEXT)";
const VAL_DDL = "CREATE TABLE IF NOT EXISTS street_valuations (id TEXT PRIMARY KEY, created_at TEXT, start TEXT, status TEXT, address TEXT, lead_source TEXT, valuation_type TEXT, branch TEXT, raw TEXT)";
const APP_DDL = "CREATE TABLE IF NOT EXISTS street_applicants (id TEXT PRIMARY KEY, kind TEXT, created_at TEXT, name TEXT, max_price INTEGER, min_beds INTEGER, lead_rating TEXT, branch TEXT, raw TEXT)";
const OFF_DDL = "CREATE TABLE IF NOT EXISTS street_offers (id TEXT PRIMARY KEY, kind TEXT, created_at TEXT, address TEXT, amount INTEGER, status TEXT, branch TEXT, raw TEXT)";
const CON_DDL = "CREATE TABLE IF NOT EXISTS street_contacts (id TEXT PRIMARY KEY, created_at TEXT, name TEXT, email TEXT, phone TEXT, status TEXT, raw TEXT)";
function mView(db, d, inc) { const a = d.attributes || {}; return db.prepare("INSERT INTO street_viewings (id,created_at,start,end_at,viewing_type,status,address,branch,property_id,raw) VALUES (?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET start=excluded.start,end_at=excluded.end_at,viewing_type=excluded.viewing_type,status=excluded.status,address=excluded.address,branch=excluded.branch,property_id=excluded.property_id,raw=excluded.raw").bind(d.id, _t(a.created_at), _t(a.start), _t(a.end), _t(a.viewing_type), _t(a.status), _t(a.address || a.public_address), _t(_branch(d, inc)), _relId(d, "property") || a.property_uuid || null, JSON.stringify(d)); }
function mVal(db, d, inc) { const a = d.attributes || {}; return db.prepare("INSERT INTO street_valuations (id,created_at,start,status,address,lead_source,valuation_type,branch,raw) VALUES (?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET start=excluded.start,status=excluded.status,address=excluded.address,lead_source=excluded.lead_source,valuation_type=excluded.valuation_type,branch=excluded.branch,raw=excluded.raw").bind(d.id, _t(a.created_at), _t(a.start), _t(a.status || a.custom_status), _t(a.address), _t(a.lead_source), _t(a.valuation_type), _t(_branch(d, inc)), JSON.stringify(d)); }
...
function schemaRes(name, path, fields, opts) {
  const tbl = "street_" + name; // physical table is source-prefixed; resource keeps its logical name
  const cols = ["id"].concat(fields).concat(["raw"]);
  const ddl = "CREATE TABLE IF NOT EXISTS " + tbl + " (" + cols.map(function (c) { return '"' + c + '"' + (c === "id" ? " TEXT PRIMARY KEY" : ""); }).join(", ") + ")";
  ...
  const sql = "INSERT INTO " + tbl + " (" + colList + ") VALUES (" + ph + ") ON CONFLICT(id) DO UPDATE SET " + upd;
   ...
   try {
    ...
    if (path === "/api/hub" && request.method === "GET") {
      ...
      try {
        ...
        pipeline.viewings = await cntq("SELECT COUNT(*) AS n FROM street_viewings");
        pipeline.viewings_week = await cntq("SELECT COUNT(*) AS n FROM street_viewings WHERE created_at >= ?", wk);
        pipeline.valuations = await cntq("SELECT COUNT(*) AS n FROM street_valuations");
        pipeline.valuations_week = await cntq("SELECT COUNT(*) AS n FROM street_valuations WHERE created_at >= ?", wk);
        pipeline.applicants = await cntq("SELECT COUNT(*) AS n FROM street_applicants");
        pipeline.contacts = await cntq("SELECT COUNT(*) AS n FROM street_contacts");
      } catch (_) {}
    ...
    if (path === "/api/contact-hub" && request.method === "GET") {
      ...
      // Call recordings the AI tied to this person (via the call it matched). Their transcripts/summaries surface here.
      let recordings = [];
      try { const rr = await db.prepare("SELECT id, title, summary, sentiment, category, next_step, created_at FROM phone_recordings WHERE contact_id = ? ORDER BY created_at DESC LIMIT 50").bind(contact.id).all(); recordings = (rr && rr.results) || []; } catch (_) {}
      return respond(JSON.stringify({ ok: true, found: true, contact: det, byKind: byKind, roles: roles, properties: properties, related: related, enquiries: enq, applicants: apps, calls: calls, recordings: recordings }), 200, J);
    }
    ...
    if (path === "/api/recording-upload" && request.method === "POST") {
      ...
      if (transcript) {
        try {
          const schema = { type: "object", properties: { title: { type: "string" }, category: { type: "string" }, sentiment: { type: "string" }, caller_type: { type: "string" }, summary: { type: "string" }, people: { type: "array", items: { type: "string" } }, property_address: { type: ["string", "null"] }, solicitor_firm: { type: ["string", "null"] }, action_items: { type: "array", items: { type: "string" } }, next_step: { type: "string" }, outcome: { type: "string" }, urgency: { type: "string" }, key_topics: { type: "array", items: { type: "string" } }, deadline: { type: ["string", "null"] }, risk: { type: ["string", "null"] } }, required: ["title", "category", "sentiment", "summary", "action_items", "next_step"] };
          const ex = await env.AI.run("@cf/meta/llama-3.1-8b-instruct-fp8-fast", { messages: [
            { role: "system", content: "You are an analyst for a UK estate agency. Extract structured facts from the call transcript. category is one of: Sale progression, Viewing, Valuation, Offer, Maintenance, Tenancy, Complaint, General. sentiment is Positive, Neutral or Negative. caller_type is Vendor, Buyer, Landlord, Tenant, Solicitor, Contractor or Other. summary is 2-3 plain sentences. property_address is the single property/address discussed, or null. solicitor_firm is a solicitor or conveyancer firm/person mentioned, or null. action_items are short follow-up actions for staff. next_step is the single most important next action in a few words. key_topics are 2-4 short tags. deadline is any date/timeframe mentioned, or null. risk is a short flag only if something needs attention (e.g. 'Vendor frustrated', 'Chain at risk', 'Complaint'), otherwise null. urgency is High, Medium or Low." },
            { role: "user", content: transcript.slice(0, 6000) }
          ], max_tokens: 700, temperature: 0.2, response_format: { type: "json_schema", json_schema: schema } });
      ...
      const solFirm = (meta.solicitor_firm && typeof meta.solicitor_firm === "string") ? meta.solicitor_firm : null;
      let callId = null, propId = null, solId = null, contactId = null;
      const newCols = ["title TEXT", "category TEXT", "sentiment TEXT", "caller_type TEXT", "people TEXT", "property_address TEXT", "property_id TEXT", "action_items TEXT", "outcome TEXT", "urgency TEXT", "next_step TEXT", "key_topics TEXT", "deadline TEXT", "risk TEXT", "solicitor_firm TEXT", "solicitor_id TEXT", "contact_id TEXT"];
      try {
        ...
        for (const col of newCols) { try { await db.prepare("ALTER TABLE phone_recordings ADD COLUMN " + col).run(); } catch (_) {} }
        if (num) { const nn = normP(num); const c = await db.prepare("SELECT id, contact_id FROM phone_calls WHERE customer_norm = ? ORDER BY started_at DESC LIMIT 1").bind(nn).first(); if (c) { callId = c.id; contactId = c.contact_id || null; } }
        if (propAddr) { const frag = propAddr.replace(/,.*$/, "").trim().slice(0, 22); if (frag.length >= 4) {
          try { const p = await db.prepare("SELECT id FROM listings WHERE address LIKE ? LIMIT 1").bind("%" + frag + "%").first(); if (p) propId = p.id; } catch (_) {}
          if (!propId) { try { const v = await db.prepare("SELECT property_id FROM street_viewings WHERE address LIKE ? AND property_id IS NOT NULL LIMIT 1").bind("%" + frag + "%").first(); if (v && v.property_id) propId = v.property_id; } catch (_) {} }
        } }
        if (solFirm) { try { const s = await db.prepare("SELECT id FROM street_solicitors WHERE full_name LIKE ? OR raw LIKE ? LIMIT 1").bind("%" + solFirm.slice(0, 16) + "%", "%" + solFirm.slice(0, 16) + "%").first(); if (s) solId = s.id; } catch (_) {} }
        await db.prepare("INSERT INTO phone_recordings (id,r2_key,bytes,call_id,customer_number,transcript,summary,title,category,sentiment,caller_type,people,property_address,property_id,action_items,outcome,urgency,next_step,key_topics,deadline,risk,solicitor_firm,solicitor_id,contact_id,error,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET r2_key=excluded.r2_key,bytes=excluded.bytes,transcript=excluded.transcript,summary=excluded.summary,title=excluded.title,category=excluded.category,sentiment=excluded.sentiment,caller_type=excluded.caller_type,people=excluded.people,property_address=excluded.property_address,property_id=excluded.property_id,action_items=excluded.action_items,outcome=excluded.outcome,urgency=excluded.urgency,next_step=excluded.next_step,key_topics=excluded.key_topics,deadline=excluded.deadline,risk=excluded.risk,solicitor_firm=excluded.solicitor_firm,solicitor_id=excluded.solicitor_id,contact_id=excluded.contact_id,error=excluded.error")
          .bind(id, r2key, buf.byteLength, callId, num, transcript, summary, _t(meta.title), _t(meta.category), _t(meta.sentiment), _t(meta.caller_type), arr(meta.people), propAddr, propId, arr(meta.action_items), _t(meta.outcome), _t(meta.urgency), _t(meta.next_step), arr(meta.key_topics), _t(meta.deadline), _t(meta.risk), solFirm, solId, contactId, aerr, new Date().toISOString()).run();
    ...
    if (path === "/api/recordings" && request.method === "GET") {
      ...
      for (const col of ["title TEXT", "category TEXT", "sentiment TEXT", "caller_type TEXT", "people TEXT", "property_address TEXT", "property_id TEXT", "action_items TEXT", "outcome TEXT", "urgency TEXT", "next_step TEXT", "key_topics TEXT", "deadline TEXT", "risk TEXT", "solicitor_firm TEXT", "solicitor_id TEXT", "contact_id TEXT"]) { try { await db.prepare("ALTER TABLE phone_recordings ADD COLUMN " + col).run(); } catch (_) {} }
      let rows = []; try { const r = await db.prepare("SELECT pr.id, pr.bytes, pr.call_id, pr.customer_number, pr.transcript, pr.summary, pr.title, pr.category, pr.sentiment, pr.caller_type, pr.people, pr.property_address, pr.property_id, pr.action_items, pr.outcome, pr.urgency, pr.next_step, pr.key_topics, pr.deadline, pr.risk, pr.solicitor_firm, pr.solicitor_id, pr.contact_id, pr.error, pr.created_at, l.address AS property_label, c.name AS contact_name, s.full_name AS solicitor_name FROM phone_recordings pr LEFT JOIN listings l ON l.id = pr.property_id LEFT JOIN street_contacts c ON c.id = pr.contact_id LEFT JOIN street_solicitors s ON s.id = pr.solicitor_id ORDER BY pr.created_at DESC LIMIT 200").all(); rows = (r && r.results) || []; } catch (_) {}
      const cats = {}, sents = {}; const actions = []; const risks = [];
      rows.forEach(function (r) { if (r.category) cats[r.category] = (cats[r.category] || 0) + 1; if (r.sentiment) sents[r.sentiment] = (sents[r.sentiment] || 0) + 1; if (r.risk) risks.push({ text: r.risk, id: r.id, title: r.title || null, property_id: r.property_id || null }); try { (JSON.parse(r.action_items || "[]") || []).forEach(function (a) { actions.push({ text: a, id: r.id, title: r.title || null, property_id: r.property_id || null, contact_id: r.contact_id || null }); }); } catch (_) {} });
      return respond(JSON.stringify({ ok: true, rows: rows, r2: !!env.RECORDINGS, cats: cats, sents: sents, actions: actions, risks: risks }), 200, J);
    }
    ...
    if (path === "/api/sync-status" && request.method === "GET") {
      ...
      for (const t of tables) {
        ...
        try { if (env.gr_estates) { const c = await env.gr_estates.prepare("SELECT COUNT(*) AS n FROM " + phys(t[0])).first(); count = (c && c.n) || 0; } } catch (_) {}
        ...
        if (dateCol[t[0]] && env.gr_estates) { try { const c2 = await env.gr_estates.prepare("SELECT COUNT(*) AS n FROM " + phys(t[0]) + " WHERE datetime(" + dateCol[t[0]] + ") >= datetime('now','start of day')").first(); today = (c2 && c2.n) || 0; } catch (_) {} }
    ...
    if (path === "/api/data-audit" && request.method === "GET") {
      ...
      for (const [name, path] of list) {
        ...
        try {
          ...
          try { if (env.gr_estates) { const c = await env.gr_estates.prepare("SELECT COUNT(*) AS n FROM " + phys(name)).first(); localCount = (c && c.n) || 0; } } catch (_) {}
          for (const rec of recs) {
            let row = null; try { if (env.gr_estates) row = await env.gr_estates.prepare("SELECT raw FROM " + phys(name) + " WHERE id = ?").bind(rec.id).first(); } catch (_) {}
    ...
    if (path === "/api/hub-search" && request.method === "GET") {
      ...
      try {
        const r = await db.prepare("SELECT id,name,email,phone,created_at FROM street_contacts WHERE name LIKE ? OR email LIKE ? OR phone LIKE ? ORDER BY created_at DESC LIMIT 6").bind(like, like, like).all();
      ...
      try {
        const r = await db.prepare("SELECT id,address,status,start,created_at,property_id FROM street_viewings WHERE address LIKE ? ORDER BY COALESCE(start,created_at) DESC LIMIT 5").bind(like).all();
      ...
      try {
        const r = await db.prepare("SELECT id,address,status,start,created_at FROM street_valuations WHERE address LIKE ? ORDER BY COALESCE(start,created_at) DESC LIMIT 5").bind(like).all();
